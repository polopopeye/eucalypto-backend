...
const firestore_providers_1 = require("./firestore.providers");
let FirestoreModule = FirestoreModule_1 = class FirestoreModule {
    static forRoot(options) {
        const optionsProvider = {
            provide: firestore_providers_1.FirestoreOptionsProvider,
            useFactory: options.useFactory,
            inject: options.inject,
        };
        const dbProvider = {
            provide: firestore_providers_1.FirestoreDatabaseProvider,
            useFactory: (config) => new firestore_1.Firestore(config),
            inject: [firestore_providers_1.FirestoreOptionsProvider],
        };
        const collectionProviders = firestore_providers_1.FirestoreCollectionProviders.map((providerName) => ({
            provide: providerName,
            useFactory: (db) => db.collection(providerName),
            inject: [firestore_providers_1.FirestoreDatabaseProvider],
        }));
        return {
            global: true,
